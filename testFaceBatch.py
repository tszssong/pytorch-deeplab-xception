...
from dataloaders.datasets import cityscapes, coco, sweeper, face, combine_dbs, pascal, sbd
from torch.utils.data import DataLoader
def test(args):
    
    val_set = face.faceSegmentation(args, split='val')
    ...
    model = DeepLab(num_classes=2,
                    ...
                    output_stride=16)
    model = model.cuda()
    checkpoint = torch.load('run/face/deeplab-resnet/model_best.pth.tar')
    ...
    for i, sample in enumerate(val_loader):
        ...
        saveshape = 300
        show_gt = cv2.resize(gt, (saveshape,saveshape))
        show_pr = cv2.resize(pred, (saveshape,saveshape))
        show_in = cv2.resize(input, (saveshape,saveshape))
        # print(show_in.shape, show_gt.shape, show_pr.shape) 
        htitch = np.hstack((show_gt, show_in[:,:,0], show_pr)) #input has 3 same channels 
        cv2.imshow("h", htitch)
        cv2.waitKey()
        # cv2.imwrite('/home/ubuntu/zms/data/coco/out/%04d.bmp'%i,htitch)
